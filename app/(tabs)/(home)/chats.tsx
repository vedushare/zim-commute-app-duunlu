...
function SkeletonRow({ colors }: { colors: ReturnType<typeof useThemeColors> }) {
  ...
  return (
    <Animated.View style={[styles.skeletonRow, { opacity, backgroundColor: colors.surface }]}>
      <View style={[styles.skeletonAvatar, { backgroundColor: colors.surfaceSecondary }]} />
      <View style={styles.skeletonContent}>
        <View style={[styles.skeletonLine, { width: '55%', backgroundColor: colors.surfaceSecondary }]} />
        <View style={[styles.skeletonLine, { width: '80%', marginTop: 8, backgroundColor: colors.surfaceSecondary }]} />
      </View>
    </Animated.View>
  );
}
...
export default function ChatsScreen() {
  ...
  const loadRooms = useCallback(async (isRefresh = false) => {
    console.log('[ChatsScreen] Loading chat rooms, isRefresh:', isRefresh);
    if (!isRefresh) setLoading(true);
    setError(null);
    try {
      const data = await getChatRooms();
      setRooms(data);
      console.log('[ChatsScreen] Loaded', data.length, 'rooms');
    } catch (err: any) {
      console.error('[ChatsScreen] Failed to load rooms:', err.message);
  ...
  const handleRefresh = useCallback(() => {
    console.log('[ChatsScreen] Pull-to-refresh triggered');
    setRefreshing(true);
    loadRooms(true);
  }, [loadRooms]);

  const handleRoomPress = useCallback((room: ChatRoom) => {
    const otherPartyId = user?.id === room.driver_id ? room.passenger_id : room.driver_id;
    const otherRole = user?.id === room.driver_id ? 'Passenger' : 'Driver';
    console.log('[ChatsScreen] Tapped room:', room.id, 'other party role:', otherRole);
      ...
      <SafeAreaView style={[styles.container, { backgroundColor: C.background }]}>
        <View style={styles.centeredState}>
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              console.log('[ChatsScreen] Sign in button pressed');
      ...
      <SafeAreaView style={[styles.container, { backgroundColor: C.background }]}>
        ...
        <View style={styles.centeredState}>
          <View style={[styles.emptyIconCircle, { backgroundColor: 'rgba(239,68,68,0.1)' }]}>
            <MessageCircle size={32} color="#EF4444" strokeWidth={1.5} />
          </View>
          ...
          <TouchableOpacity
            style={[styles.ctaButton, { backgroundColor: C.primary }]}
            onPress={() => {
              console.log('[ChatsScreen] Retry button pressed');
              loadRooms();
            }}
          >
        ...
        >
          ...
          <View style={styles.roomContent}>
            <View style={styles.roomTopRow}>
              <Text style={[styles.roomName, { color: C.text }, hasUnread && styles.roomNameBold]} numberOfLines={1}>
                {otherRole}
              </Text>
              {lastMsgTime ? (
            ...
            <View style={styles.roomBottomRow}>
              <Text
                style={[styles.lastMessage, { color: hasUnread ? C.text : C.textSecondary }]}
                numberOfLines={1}
                ellipsizeMode="tail"
              >
                {lastMsgPreview}
